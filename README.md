...
Gmail 메일함에 쌓인 약속·일정 정보를 AI가 자동으로 추출해 캘린더에 등록해주는
...
| 구분 | 내용 |
| --- | --- |
| 문제 | 컴포넌트 간 결합도가 높아 수정 시 연쇄 오류가 발생, 과도한 props drilling. 날짜·시간 형식이 컴포넌트마다 제각각이라 캘린더/일정 상세/AI 요약에서 데이터 불일치가 자주 발생 |
| 원인 | 초기 개발 시 빠른 구현 중심으로 상태·로직 규칙을 정리하지 않음. 날짜 처리, 공통 유틸, 상태 구조가 통일되지 않은 채 코드가 누적됨 |
| 해결 | `Context` 분리(State/Dispatch)로 역할을 명확히 하고, `Reducer + payload` 패턴으로 상태와 로직을 완전히 분리. 지도·날씨·일정 정렬 등 중복 로직은 `utils`로 추출해 형식을 일관화. `<ScheduleProvider>` + `useScheduleState()` / `useScheduleDispatch()` Hook으로 컴포넌트에 개별 전달하던 10개 이상의 props를 대체 |
| 배운 점 | 개발은 혼자보다 팀원과의 잦은 커뮤니케이션이 훨씬 큰 효과를 준다는 것을 체감. 서로 다른 관점에서 나온 의견들이 구조 개선에 큰 도움이 됨 |

### 인증 흐름 변경에 따른 프론트 대응 (JWT 전환)

| 구분 | 내용 |
| --- | --- |
| 문제 | 세션 기반 인증에서는 로그인 자체가 정상 동작하지 않았고, Google OAuth 로그인 이동 과정에서 세션이 증발해 로그인된 구글 계정과 사용자 연결이 불가능했음 |
| 원인 | HTTPS 프론트 ↔ HTTP 백엔드 교차 요청으로 Secure 쿠키가 차단되고, OAuth 리다이렉션 과정에서 HTTP/HTTPS 프로토콜 혼재 + 도메인 불일치로 세션 쿠키가 계속 유실됨 |
| 해결 | 프론트에서 세션 대신 로그인 성공 시 발급받은 JWT를 `localStorage`에 저장하고, 이후 모든 API 요청·구글 연동(OAuth) 요청·웹소켓 연결에 `Authorization: Bearer {token}` 헤더로 동일하게 인증 |
| 배운 점 | OAuth 리다이렉트가 포함되는 환경에서는 HTTP/HTTPS 프로토콜이 단 1회라도 어긋나면 세션 기반 인증은 작동하지 않는다. 분리 배포 환경에서는 JWT 기반 인증이 사실상 유일하게 안정적인 선택지였다 |
